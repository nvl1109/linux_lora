...
static void di0_isr_handler(void* arg);
...
int loraInit(void)
{
    ...
    if (lora_spi_init() != 0) {
        print_err(LOG_TAG "spi init failed");
        return -1;
    }

    if (gpio_init() != 0) {
        print_err(LOG_TAG "gpio init failed");
        return -1;
    }
...
{
    if (0 != gpio_reset_rfm95()) {
        print_err(LOG_TAG "reset RFM95 failed");
        return -1;
    }
    ...
    if (version != 0x12) {
        print_err(LOG_TAG "HW version 0x%x is invalid", version);
        return 0;
    }
...
void setFrequency(long frequency)
{
    ...
    print_inf("%ld ==> frf = 0x%llx", frequency, frf);
...
void setSPIFrequency(uint32_t frequency)
{
    ...
    print_err(LOG_TAG "setSPIFrequency is not implemeted");
}

void dumpRegisters()
{
    print_inf(LOG_TAG "RFM95 Registers:");
    for (int i = 0; i < 128; i++) {
        printf(LOG_TAG "addr 0x%02x = 0x%x\n", i, readRegister(i));
    }
}
